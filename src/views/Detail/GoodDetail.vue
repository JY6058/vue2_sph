          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="item in spuSaleAttrList" :key="item.id">
                ...
                <dd changepirce="0" 
                :class="{active:data.isChecked==1}" 
                v-for="data in item.spuSaleAttrValueList" 
                :key="data.id" @click="changeCheck(data,item.spuSaleAttrValueList)">
                {{data.saleAttrValueName}}
                </dd>
              </dl>
            </div>
            <div class="cartWrap">
              <div class="controls">
                <input autocomplete="off" class="itxt" v-model="goodNums" @change="changeGoodNums">
                <a href="javascript:" class="plus" @click="goodNums++">+</a>
                <a href="javascript:" class="mins" @click="goodNums>1?goodNums--:goodNums=1">-</a>
              </div>
              <div class="add">
...
<script>
  ...
  export default {
    ...
    methods:{
      ...
      async addShopcat(){
        try{
          await this.$store.dispatch('addShopcat',{skuId:this.$route.params.skuId,skuNum:this.goodNums})
          // 将数据存入sessionStoragy
          sessionStorage.setItem('addshopcart',JSON.stringify(this.skuInfo))
          // 跳转页面
          this.$router.push({name:'addcartsuccess',query:{skuNum:this.goodNums}})
        }catch(error){
          alert(error.message)
        }
      }
